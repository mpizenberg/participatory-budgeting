# Notes

Potential title: Where participatory budgeting and grant funding meet, or how to effectively survey large groups of people and fairly distribute significant amount of funds.

## Ranking Voting

Ranking > Rating to force people to take decisions, mitigate max-raters, reveals what truly matters.

This ref on ranking methods is quite good: https://www.opinionx.co/blog/choosing-a-survey-ranking-method

Approaches:
- Direct ranking of all options
- Points allocation
- Pairwise comparisons
- MaxDiff (best-worst of subsets of 3-6 items)
- Conjoint analysis (e.g. brand x price x battery)

When choosing a single winner: studies show IRV and Condorcet give best results
- First-Past-the-Post (count only first choices)
- Instant-Runoff Voting (IRV, iterative eliminate lowest and reallocate ballots until one > 50%)
- Borda Count (descending points to ranks)
- Condorcet Methods: (best in pairwise contests)

Open-Source tools:
- LimeSurvey: https://github.com/LimeSurvey/LimeSurvey
- Formbricks: https://github.com/formbricks/formbricks, https://formbricks.com/docs/xm-and-surveys/surveys/question-type/ranking
- SurveyJS: https://github.com/surveyjs
- Belenios (Inria): https://www.belenios.org/
- Condorcet: https://github.com/julien-boudry/Condorcet
- RCTab: https://www.rcvresources.org/rctab/
- OpenRCV: https://github.com/cjerdonek/open-rcv

## Group Decision and Participatory Budgeting

Group decision platforms:
- Decidim: https://decidim.org/
  Particularly interesting is https://decidim.org/blog/2026-04-07-case-study-participatory-budget-2020-2023-in-barcelona/
- https://consuldemocracy.org/
- Loomio: https://www.loomio.com/how-it-works
  Really interesting Loomio history: https://www.loomio.coop/history.html
- Polis: https://pol.is/home2 (opinion mapping, different but interesting) https://github.com/compdemocracy/polis

Examples PB:
- Lisbon Lx-PB (2008-today):
  - https://journals.openedition.org/factsreports/3363#ftn17
  - https://centreforpublicimpact.org/public-impact-fundamentals/green-participatory-budgeting-lisbon-portugal/
- Decidim Barcelona (2017-today):
  - Aragon et. al. 2017, Deliberative Platform Design: The Case Study of the Online Discussions in Decidim Barcelona. https://arxiv.org/pdf/1707.06526

## Grant Funding

TODO
